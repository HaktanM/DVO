    ...
    def __init__(
        ...
        self.resize_layers = nn.ModuleList(
            [
                ...
                nn.Identity(),
                nn.Conv2d(
                    in_channels=out_channels[2], out_channels=out_channels[2], kernel_size=3, stride=2, padding=1
                ),
            ]
        )
...
class DPTHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return out # self.conv_depth(out)

    def predict(self, inputs, rescale_to=(512, 512)):
        out = self.forward_features(inputs)
        return out # self.conv_depth.predict(out, rescale_to=rescale_to)
